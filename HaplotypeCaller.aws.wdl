...
task HaplotypeCaller {
  ...
  command {
    ${gatk_path} --java-options ${java_opt} \
      ...
      -O ${gvcf_name} \
      -L ${interval_list} \
      -ip "${if defined(interval_padding) then interval_padding else 100}" \
      -contamination "${if defined(contamination) then contamination else 0}" \
      --max-alternate-alleles "${if defined(max_alt_alleles) then max_alt_alleles else 3}" \
      -ERC GVCF
  }
